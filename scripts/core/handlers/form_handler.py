...
class FormHandler:
    def __init__(self, login_token, encrypt_payload, file_path, component_json_path, sheet_name, current_ts, step_data, response_messages):
        self.login_token = login_token
        self.encrypt_payload = encrypt_payload
        self.file_path = file_path
        self.sheet_name = copy.deepcopy(sheet_name)
        self.component_json_path = component_json_path
        with open(self.component_json_path, 'r') as file:
        ...
        self.property_names = []
        self.response_messages = copy.deepcopy(response_messages)
        ...
        try:
            ...
            for row in range(0, table_no_rows):
                ...
                for col in range(0, table_num_cols):
                    ...
                    elif cell_value is not None and re.search(r"<header.*>", cell_value):
                        components_list, _, _ = self.process_header_component(
                            from_parent_itr=True,
                            cell_value=cell_value,
                            components_list=components_list)
            ...
            if step_category and step_category in ['step_category_101', 'task creation step', 'Task Creation Step']:
                response = copy.deepcopy(step_data)
            else:
                response = self.update_step_data(step_data, {"components": components_list})

            return response, self.response_messages
            ...
            for new_row in range(0, table_no_rows):
                ...
                for new_col in range(0, table_num_cols):
                    ...
                    if cell_value is not None and re.search(r"<html.*>", cell_value):
                        ...
                        _, row_list, col_list = self.process_header_component(
                            cell_value=cell_value,
                            merge_properties=merge_properties,
                            child=child,
                            current_component=current_component,
    ...
    def process_header_component(self, from_parent_itr=False, cell_value=None, components_list=None,
                               merge_properties=None, child=None, current_component=None, row_list=None,
                               col_list=None):
        try:
            # fetch static html component json
            html_components = copy.deepcopy(self.component_json.get("header"))
            content = self.step_data.get('header', '') or self.step_data.get('step_name', '')
            content = content.strip() if content else content
            # html_components["customClass"] = "text-left"
            html_components["content"] = f"<b style = \"color:white;\">{content}</b>"
    ...
    def update_step_data(self, step_data, form_data):
        try:
            ...
            if step_valid:
                ...
                if self.date_keys:
                    payload.update({"dateKeys": self.date_keys})
                decoded_payload = {}
                ...
                if self.encrypt_payload:
                    ...
                    response = requests.post(url, json=payload, headers=Secrets.headers, cookies=self.login_token)
                step_name = self.step_data.get('step_name', '')
                if response.status_code != 200:
                    self.response_messages += f"Failed to update step {step_name} \n"
                    return {'message': f"Failed to update step {step_name} \n"}
                    # raise HTTPException(status_code=response.status_code, detail='Failed to update step')
                response = response.json()
                data = response.get('data', {})
                self.response_messages += f"Updated steps '{step_name}' successfully\n"
                if self.encrypt_payload:
